...
| Threat                     | Mitigation                                                                                                                                         |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Password DB leak           | HMAC-SHA256 **pepper** applied before scrypt (read from a Docker secret via the `PEPPER_FILE` env variable); hashes useless without the app secret |
...
1. Create Docker secrets (see `src/env.ts` for all keys):

   ```bash
   mkdir -p secrets
   echo "change-me-in-production" > secrets/pepper.txt
   echo "your-email-api-key" > secrets/email_key.txt
   ```

   Configure the remaining env vars in a `.env` file:
   ...
   > **Warning:** the pepper is mixed into every password hash (HMAC before scrypt). Changing `secrets/pepper.txt` after users are registered invalidates **all** stored hashes, nobody can log in until passwords are reset.
...
2. Start the full stack (Node API + Caddy):
   ...
   > The included `compose.override.yaml` automatically activates in development, it targets the `dev` build stage and bind-mounts the source tree for hot-reload.
