import { type ScryptOptions, createHmac, timingSafeEqual } from "node:crypto";
import { randomBytesAsync, scryptAsync } from "./utils.js";
...
class Password {
  ...
  async hash(password: string) {
    ...
    return (
      `scrypt$v=1$norm=${this.NORM}` +
      `$N=${this.SCRYPT_OPTIONS.N},r=${this.SCRYPT_OPTIONS.r},p=${this.SCRYPT_OPTIONS.p}` +
  ...
  async verify(password: string, password_hash: string) {
    try {
      const { stored_norm, stored_options, stored_dk, stored_salt } =
